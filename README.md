# 我的常用软件与硬件清单

这是一份长期自用的工具清单，记录我在 `Mac`、`Windows`、手机和日常硬件上的常用选择。  
目标不是“列全”，而是尽量保留那些真的好用、能长期留下来的东西。

## 下载入口

优先选择官方或稳定渠道：

- [腾讯软件中心](https://pc.qq.com/)
- [联想软件管家](https://lestore.lenovo.com/)
- [Softonic](https://en.softonic.com/)
- [Windows 10 官方下载](https://www.microsoft.com/zh-cn/software-download/windows10)
- [Windows 11 官方升级](https://www.microsoft.com/zh-cn/software-download/windows11)
- [蓝奏云盘](https://up.woozooo.com/)：常见软件外链
- [荔枝](https://store.lizhi.io/)：正版软件购买

如果是联想电脑，能在联想软件管家里下载的，优先从那里下，通常更省心。

## Mac 与 iPhone

### 核心推荐

- 浏览器
  - [Edge](https://www.microsoftedgeinsider.com/en-us/download/)：跨设备同步方便，书签同步比较稳定
  - Chrome：体验依然很好，但登录问题有时比较折腾
- 社交软件
  - [QQ](https://im.qq.com/download/)
  - [微信](https://weixin.qq.com/)
- 输入法
  - [微信输入法](https://z.weixin.qq.com/)：多平台同步稳定，是我目前更偏向的选择
- 文件管理
  - [QSpace](https://qspace.awehunt.com/zh-cn/)：适合替代访达，重命名和解压都比较顺手
- 下载工具
  - [NDM](http://www.neatdownloadmanager.com/index.php/en/)：轻量、直接，和浏览器搭配好用
- PDF 工具
  - [PDF24](https://tools.pdf24.org/zh/)：在线工具够全
  - [PDFgear](https://www.pdfgear.com/zh/)：有客户端，搭配用更方便
- 办公软件
  - [WPS](https://www.wps.cn/)：暂时还是很难替代
- 截图软件
  - [Snipaste](https://zh.snipaste.com/)：稳定、顺手，基本必装
- 视频播放器
  - [IINA](https://iina.io/)：Mac 上相对完整、成熟的免费播放器
- 解压软件
  - [Keka](https://www.keka.io/zh-cn/)：免费，习惯之后很好用
- 互传工具
  - [LocalSend](https://localsend.org/zh-CN)：跨平台互传方便，但文件多时性能一般
- 鼠标工具
  - [Scroll Reverser](https://pilotmoon.com/scrollreverser/)：滚动方向调整非常顺手
  - [中文说明](https://github.com/TomIsion/iScroll-Chinese-README?tab=readme-ov-file)
- iPhone 管理
  - [iMazing](https://www.imazingchina.com/)：管理 iOS 设备比较完整，适合买断

### 其他我会装的工具

- Aldente：电池管理
- Bartender：整理右上角菜单栏图标
- [AnyTXT](https://anytxt.net/)：本地全文搜索
- PhotoBulk：图片批量加水印
- [隐藏刘海](https://topnotch.app/)
- [Fliqlo](https://fliqlo.com/screensaver/)：时间屏保

### 目前仍不够满意的空缺

- 重命名软件：还没有找到能替代 Windows 上 ReNamer 的方案
- 图片快速处理：还没有遇到比光影魔术手更顺手的
- 视频播放器：暂时没有 PotPlayer 级别的替代品

## Windows PC 软件

### 装机优先级最高的一批

- 浏览器
  - [Chrome](https://www.google.cn/chrome/)：建议安装 64 位版本
  - [Edge](https://www.microsoftedgeinsider.com/en-us/download/)：同步体验更方便
- 社交软件
  - [QQ](https://im.qq.com/download/)
  - [微信](https://weixin.qq.com/)
- 输入法
  - [手心输入法](http://www.xinshuru.com/index.html?p=win)
- 下载工具
  - [NDM](http://www.neatdownloadmanager.com/index.php/en/)：免费、够用、轻便
  - [迅雷绿色版](https://www.aliyundrive.com/s/GVsDt8iEnTZ)：有些场景还是绕不开
- 系统安全与卸载
  - [火绒](https://www.huorong.cn/)
  - [Geek Uninstaller](https://geekuninstaller.com/download)
- 文档与办公
  - [SumatraPDF](https://www.sumatrapdfreader.org/free-pdf-reader)
  - [PDF24](https://tools.pdf24.org/zh/)
  - [WPS v11.8.2.11019](https://www.aliyundrive.com/s/W8sT57gsbJ2)：支持经典模式
- 效率工具
  - [PowerToys](https://github.com/microsoft/PowerToys)
  - [Snipaste](https://zh.snipaste.com/)
  - [QuickLook](https://www.microsoft.com/zh-cn/p/quicklook/9nv4bs3l1h4s?ocid=badge&rtc=1&activetab=pivot:overviewtab/)
  - [CopyQ](https://hluk.github.io/CopyQ/)
  - [OneCommander](https://onecommander.com/)
- 音视频
  - [PotPlayer](https://wwzt.lanzoul.com/iGOC20if05mb)
  - [PotPlayer 官网](https://potplayer.daum.net/)
  - [PotPlayer 国内镜像](https://potplayer.tv/?lang=zh_CN)
  - [K-Lite Codec Pack](http://www.codecguide.com/configuration_tips.htm?version=1845)
  - [ShanaEncoder](https://wwzt.lanzoul.com/iLxeQ2ic27kh)
- 笔记与词典
  - [欧路词典](https://dict.eudic.net/)
  - [Joplin](https://joplinapp.org/)
  - [OneNote](https://www.onenote.com/?public=1&wdorigin=ondcauth2&wdorigin=ondc)

### 我会继续保留的其他工具

- [Watt Toolkit / Steam++](https://steampp.net/)：有时能帮忙处理网络访问
- [PasteEx](https://github.com/huiyadanli/PasteEx)
- [Handshaker](https://www.smartisan.com/apps/#/handshaker)
- [AMD 驱动升级](https://www.amd.com/zh-hans/support)
- [Adobe 全家桶](https://www.aliyundrive.com/s/kyRiDLhoBGW)
- [落雪音乐](https://github.com/lyswhut/lx-music-desktop#readme)
- [SoftCnKiller](https://wwzt.lanzoul.com/isW8B2ic2r1i)

### 网盘同步

- [坚果云](https://www.jianguoyun.com/s/downloads/)：支持 WebDAV
- [百度网盘](https://pan.baidu.com/download)
- [阿里云盘](https://www.aliyundrive.com/)
- [蓝奏云](https://www.lanzoux.com/)
- [夸克网盘](https://www.quark.cn/)

### 录屏软件

- [OBS](https://obsproject.com/)：免费开源，功能最强，但有学习成本
- [Captura](https://captura.updatestar.com/)
- [oCam](https://pan.baidu.com/s/1kViBPslgr45O291ed9cu2g/)

### 文件处理

- [Office Tool Plus](https://otp.landian.la/zh-cn/)
- [ReNamer](http://www.den4b.com/products/renamer)
- [Everything](https://www.voidtools.com/downloads/)
- [FileLocator Pro](https://wwa.lanzoui.com/ipHN6regcxc)
- [AllDup](https://www.alldup.info/en_download_alldup.php)
- [NewFolderWizard](https://wwp.lanzoul.com/iiabi03qrb1e)
- [DropIt](http://www.dropitproject.com/)
- [Bandizip 6.25](https://wwe.lanzoux.com/ialDVihl5sj)
- [7-Zip](https://www.7-zip.org/)
- [数据恢复工具](https://pjs.lanzoux.com/ibipeud/)

### 图片相关

- [FastStone Image Viewer](https://wwp.lanzoul.com/icZVF1pc62yb)：适合按文件夹浏览图片和视频，也支持 HEIC
- [IrfanView](https://www.irfanview.com/)：批量处理、转换 HEIC 很方便
- [HoneyView](https://www.bandisoft.com/honeyview/)
- [Billfish](https://www.billfish.cn/)
- [Picasa](https://pjs.lanzoux.com/ibipdvi/)
- [光影魔术手](https://wwzt.lanzoul.com/iuDsN28asdna)

### 装机与系统工具

- [HelloWindows](https://hellowindows.cn/)
- [系统库](https://www.xitongku.com/)
- [DiskGenius 单文件版](https://www.aliyundrive.com/s/7wiWbXsPSBT)
- [驱动人生万能网卡版](https://wwp.lanzoul.com/i0Urz06m6kvg)
- [驱动注入工具](https://www.aliyundrive.com/s/nwzRYuVP9Yr)
- [Ventoy](https://www.ventoy.net/cn/index.html)
- [微 PE](http://www.wepe.com.cn/download.html)
- [Rufus](https://rufus.ie/zh_CN.html)

常见问题：

- U 盘启动如果出现 `Verification failed`，可尝试在 BIOS 中关闭 `Secure Boot`

### 其他小工具

- [SpaceSniffer](http://www.uderzo.it/main_products/space_sniffer/index.html)
- [ContextMenuManager](https://github.com/BluePointLilac/ContextMenuManager/releases)
- [Quicker](https://getquicker.net/)
- [Spacedesk](https://spacedesk.net/)
- [Simplenote](https://simplenote.com/)
- [Lively Wallpaper](https://apps.microsoft.com/store/detail/lively-wallpaper/9NTM2QC6QWS7?hl=en-us&gl=US)
- [Wnr](https://getwnr.com/zh/)
- [AIDA64](https://pjs.lanzoul.com/ibj85eh/)
- [Inpaint](https://www.theinpaint.com/download.html)
- [Moo0 Tools](https://zhs.moo0.com/?top=https://zhs.moo0.com/best/%23tools)
- [FFmpeg](https://ffmpeg.org/)
- [Ditto](https://ditto-cp.sourceforge.io/)
- [DeskPins](https://efotinis.neocities.org/deskpins/)
- [MIUI+](https://www.mi.com/service/notebook/drivers/A53)

软媒工具：

- [设置大师](https://pjs.lanzoux.com/ibipdeb/)
- [清理大师](https://pjs.lanzoux.com/ibipdcj/)
- [美化大师](https://pjs.lanzoux.com/ibipdda/)

### CAD 与 Revit

如果 CAD 或 Revit 多次安装失败，很多时候直接重置系统比反复修补更省时间。

- [中望 CAD](https://www.zwcad.com/)
- [CAD 字体](https://wwa.lanzoui.com/iOkizqoc77g)
- [CAD 2022](https://www.aliyundrive.com/s/RCP47zLSKpE)
- [CAD 自学网](http://www.cadzxw.com/download.html/)
- [AcmeCAD](https://wwzt.lanzoul.com/iNkMT2ic0ztc)
- 天正 T20 V9.0：支持 CAD 2010 以上版本
- [Revit 2020](https://www.aliyundrive.com/s/vzLK6MD4V46)
- [Revit 2020 许可管理器问题说明](https://www.aliyundrive.com/s/9cghFT6R4gW)
- [橄榄山插件](http://www.glsbim.com/intro/42.html)
- [构件坞插件](https://www.goujianwu.com/plugin)

## 其他 Mac 软件

- [SnippetsLab](https://www.renfei.org/snippets-lab/)：代码片段管理
- Folder Peek
- Mounty
- AirDrop
- Hidden Bar
- Omi

## 手机软件

### Android 独享

- WOW 时钟
- NFC Tools Pro
- [习惯](https://github.com/iSoron/uhabits/)
- Gitme
- 小米天气
- [MiXplorer](https://wwp.lanzoul.com/b017p1i3g/)：文件管理很好用，记得安装配套插件

### iPhone

- 暂时没有特别固定、必须保留的独占推荐

### 双平台常用

- QQ、微信、淘宝、天猫、支付宝、京东、拼多多、云闪付、哔哩哔哩
- 优酷、网易云音乐、酷安、全历史、WPS、知乎、OneNote、欧路词典
- 星图、天气通、高德地图、百度地图、下厨房、最右、古文岛、百度网盘
- Joplin、微软数学、阿里云盘

### 备注

- 手心输入法在 iPhone 上偶尔会断触，但在 Mac 和 Android 上我一直在用
- 微信聊天记录可以在电脑客户端备份
- QQ 输入法使用 QQ 登录，其他很多服务我通常用微信登录

## 配件硬件推荐

### 氮化镓充电器

- 小米手机：优先原装
- iPhone：安克 30W 基本够用
- Mac：65W 绿联氮化镓

### 电池

- 7 号电池：博朗耳温枪电池
- 5 号电池：超霸 LR6

鼠标键盘自带电池通常已经很耐用，不需要频繁更换。

### 打印机

- 惠普 M126nw：无线打印扫描一体，硒鼓 `CC388A`
- [驱动下载](https://support.hp.com/cn-zh/drivers/selfservice/hp-laserjet-pro-mfp-m126-series/5396665/model/5303411?ssfFlag=true&sku=)

### 键盘

- 罗技 K380：轻薄好用，但没有数字键盘
- 机械键盘：我个人更偏向安静，不是首选

### 鼠标

- 罗技鹅卵石：安静顺手，按键用久了可能需要更换微动

### 显示器

- 超长 2K：待补充
- 4K：我现在用的是泰坦，主打性价比

### 文具

- 自动铅笔
  - 三菱 M5-559：双倍速，价格实惠
  - 三菱 M5-450：单倍速，价格实惠，我个人更喜欢单倍速
- 自动铅笔笔芯
  - 三菱防污 UL-S，0.5 HB
  - 三菱彩铅：颜色偏浅，也比较容易断
- 中性笔
  - 小米巨能写
  - 百乐 P500：手感不错，但盗版多，注意渠道

## 其他记录

### Windows 11 设置

- 系统最好保持定期整理；有些顽固问题，重置系统反而是最省时间的做法
- 取消开机密码：`Win + R` 输入 `netplwiz`，取消勾选登录要求后按提示操作

### Chrome 插件

- [插件下载网](https://crxdl.com/)
- Adblock Plus
- [Chrome 包](https://wwzt.lanzoul.com/b018bxcu/)：密码 `1111`
- [Chrome 重装后需要补的内容](https://wwzt.lanzoul.com/b018bzwle)
- [ImageAssistant](http://www.pullywood.com/ImageAssistant/)
- [Dark Reader](https://darkreader.org/)
- [Octotree](https://www.octotree.io/)
- [Violentmonkey](https://github.com/violentmonkey/violentmonkey/releases)
- [Tampermonkey](https://www.tampermonkey.net/)
- [Tampermonkey 脚本](https://www.tampermonkey.net/scripts.php?ext=dhdg)
- [哔哩哔哩过滤器](https://s.xmcp.ltd/pakkujs/?src=wext_homepage)
- [FireShot](https://getfireshot.com/)
- [哔哩哔哩下载助手](https://csser.top/)
- [腾讯交互翻译](https://transmart.qq.com/zh-CN/download)

补充：

- Chrome 同步助手 1.8 目前基本已经不可用
- 重装或卸载浏览器前，记得先备份密码、书签和插件配置

### 小技巧

- 文件批量重命名：全选后按 `F2`
- 浏览器恢复刚关闭的页面：`Ctrl + Shift + T`
- 批量新建文件夹如果出现乱码，可把文本另存为 `ANSI` 编码
- 联想一键还原配合 Windows 重置功能，处理系统问题时很有用

### 已停用或不再推荐

- [知犀](https://www.zhixi.com/)
- [Syncthing](https://syncthing.net/)
- [Listen1](https://listen1.github.io/listen1/)
- [Foobar2000 官网](https://www.foobar2000.org/)
- [Foobar2000 汉化版](https://wwp.lanzoul.com/inkKw06briwf)
- [MusicPlayer2](https://github.com/zhongyang219/MusicPlayer2)
- [PDF 补丁丁](https://www.cnblogs.com/pdfpatcher/)
- [uTools](https://u.tools/)
